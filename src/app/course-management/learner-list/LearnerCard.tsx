...
export default function LearnerCard(props: {
	learnerUTxO: LearnerReferenceUTxO;
}) {
	return (
		<div
			className='bg-slate-300 text-black p-5 rounded-ld drop-shadow'
			key={props.learnerUTxO.utxo.input.txHash}>
			...
			<ul>
				{props.learnerUTxO.data?.completedAssignments.map(
					(item: string, index) => (
