	...
	return (
		<div className='my-10 w-5/6 mx-auto grid grid-cols-4 gap-10'>
			<div className='col-span-1'>
				<h1 className='text-4xl text-primary-content py-10'>
					{andamioConfig.title} Learner Tokens
				</h1>
			</div>
			<div className='col-span-3'>
				<table className='table table-fixed bg-primary text-primary-content'>
					...
					<tbody>
						{learnerReferenceInfo.utxos.map(
							(learnerUTxO: LearnerReferenceUTxO, index) => (
								// <LearnerCard learnerUTxO={learnerUTxO} key={index} />
								<tr
									key={learnerUTxO.data?.contributorAlias}
									className='text-lg'>
									<th>{learnerUTxO.data?.contributorAlias}</th>
									<th>{learnerUTxO.data?.learnerInfo}</th>
									<th className='text-center'>
										{learnerUTxO.data?.completedAssignments.length}
									</th>
									<th className='text-center'>
										{learnerUTxO.data?.completedAssignments.pop()}
									</th>
								</tr>
							)
						)}
