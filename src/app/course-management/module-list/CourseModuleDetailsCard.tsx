...
export default function CourseModuleDetailsCard(props: {
	...
	const moduleOnChain = getOnchainCourseModule(
		props.courseJSON,
		props.onChainModules
	);
	return (
		<div className='flex flex-col my-5' key={props.courseJSON.id}>
			<div className='flex flex-row py-5 px-5 justify-between bg-secondary text-secondary-content rounded-lg drop-shadow'>
			...
			<div className='p-5 text-xl bg-secondary text-primary rounded-lg drop-shadow my-2'>
				<ul className='list-disc pl-5 rounded-lg drop-shadow'>
			...
			{moduleOnChain?.data?.moduleId && (
				<div>
					{props.courseJSON.id == moduleOnChain?.data?.moduleId ? (
						<div className='bg-success p-2 text-primary rounded-lg drop-shadow'>
							Module Id matches module token name
						</div>
					) : (
						<div className='bg-warning p-2 text-primary rounded-lg drop-shadow'>
							Module Id does not match module token name
						</div>
					)}
				</div>
			)}
			<div className='bg-primary text-primary-content p-5 rounded-lg drop-shadow'>
				<p className='pb-3'>
					Assignment: {getAssignment(props.courseJSON, props.onChainModules)}
				</p>
				<pre className='text-sm'>
					Module Content Hash: {hashCourseModule(props.courseJSON)}
				</pre>
				<pre className='text-sm'>
					Module Onchain Hash: {moduleOnChain?.data?.moduleHash}
				</pre>
				<pre className='text-sm'>
					Module CS: {moduleOnChain?.data?.moduleCS}
				</pre>
				{moduleOnChain && (
					<div className='flex flex-row gap-6 py-3'>
						<div>
							{<CommitToAssignmentModal selectedModuleUTxO={moduleOnChain} />}
						</div>
						<Link
							href={`/course/module/${moduleOnChain.data?.moduleId}/overview`}>
							<button
						...
						<div>
							{<AddLearnerPolicyIDModal selectedModuleUTxO={moduleOnChain} />}
						</div>
