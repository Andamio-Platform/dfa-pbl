...
export default function CourseModuleDetailsCard(props: {
	...
	const modulePublished = getPublishedCourseContent(
		props.courseJSON,
	...
	return (
		<div
			className='card border border-primary text-secondary-content shadow-xl grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 my-5'
			key={props.courseJSON.id}>
			<div className='md:col-span-2 lg:col-span-3 p-4'>
				<div className='flex flex-col py-1 justify-between'>
					<h5 className='text-accent text-sm font-bold'>
						Module {props.courseJSON.id}
					</h5>
						...
						{modulePublished ? (
							<>
								<Link href={`/course/module/${props.courseJSON.id}/overview`}>
									<button className='btn btn-sm btn-success hover:scale-105'>
										Learn
									</button>
								</Link>
								{moduleOnChain && (
			...
			<div className='w-full col-span-1 row-span-2 text-primary-content'>
				<div className='p-5'>
					<pre className='pb-1 text-sm'>Assignment:</pre>
					<pre className='pb-3 text-sm'>
						{getAssignment(props.courseJSON, props.onChainModules)}
					</pre>

					<div className='flex flex-col gap-5 mt-5'>
						<div
							className='tooltip hover:scale-105'
							data-tip={`Click to view Assignment ${props.courseJSON.id}`}>
							<Link
								href={`/course/module/${props.courseJSON.id}/assignment${props.courseJSON.id}`}>
								<Image
									src='/icons/assignment-primary2-sm.png'
									width={30}
									height={30}
									alt='assignment'
								/>
						...
						<a href='https://andamio.notion.site/Andamio-On-Chain-Course-Modules-3d04a6dc28684f019e75e22ea94ed9b7'>
							<div
								...
								<Image
									src='/icons/validate-primary-sm.png'
									width={30}
									height={30}
									alt='validate'
								/>
					...
					<>
						{props.courseJSON.id == moduleOnChain?.data?.moduleId ? (
							<div className='bg-success p-2 font-mono rounded-b-[0.2rem]'>
								Module is deployed on-chain and Module Id matches module token
								name
							</div>
						) : (
							<div className='bg-warning p-2 rounded-b-[0.2rem]'>
