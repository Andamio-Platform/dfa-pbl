...
import {
	...
	prepareBurnLearnerReferenceTokenTx
} from "@andamiojs/core";
...
const BurnLearnerTokens = (props: {
	...
	if (connected) {
		const handleClick = async () => {
			try {
				const BURN_LEARNER_REFERENCE_TOKEN =
					await prepareBurnLearnerReferenceTokenTx(
						wallet,
						andamioConfig,
			...
			<>
				...
				<div className='flex flex-col p-10 w-max'>
					<div className='mb-4 text-lg'>
						You will burn your learner token and the reference token.
					</div>

					<button
						onClick={handleClick}
						className='btn btn-wide btn-error rounded-lg drop-shadow text-primary-content hover:bg-primary hover:scale-105'>
						Confirm Burn
					</button>
				</div>
			</>
